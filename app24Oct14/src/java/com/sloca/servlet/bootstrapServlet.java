 ...
 * @author Langxin
 */
@WebServlet(name = "bootstrapServlet", urlPatterns = {"/bootstrapServlet.do"})
@MultipartConfig
...
public class bootstrapServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (".zip".equals(fileName.substring(fileName.length() - 4))) {
            ...
            try {
                locationLookupError = upload.uploadFileToDatabase("location-lookup.csv", unzipPath);
            } catch (Exception ex) {
                Logger.getLogger(UploadServlet.class.getName()).log(Level.SEVERE, "servlet", ex);
            }

            try {
                demographicsError = upload.uploadFileToDatabase("demographics.csv", unzipPath);
            } catch (Exception ex) {
                Logger.getLogger(UploadServlet.class.getName()).log(Level.SEVERE, "servlet", ex);
            }

            try {
                locationError = upload.uploadFileToDatabase("location.csv", unzipPath);
            } catch (Exception ex) {
                Logger.getLogger(UploadServlet.class.getName()).log(Level.SEVERE, "servlet", ex);
            }
